...
(function installWayneActionIcons(){
  ...
  const decorate=root=>root.querySelectorAll?.('.simple-actions a,.simple-actions button,.nav-actions a,.nav-actions button,.dash-nav a,.dash-nav button,.admin-menu a,.admin-menu button,main button:not(.wayne-dialog-actions button)').forEach(element=>{if(element.querySelector(':scope > .wayne-nav-icon,:scope > .wayne-action-icon'))return;const key=choose(element);if(!key)return;const nav=!!element.closest('.simple-actions,.nav-actions,.dash-nav,.admin-menu'),span=document.createElement('span');span.className=nav?'wayne-nav-icon':'wayne-action-icon';span.setAttribute('aria-hidden','true');span.innerHTML=`<svg viewBox="0 0 24 24">${icons[key]}</svg>`;element.prepend(span)});
...
if (!document.querySelector('link[href="premium-pages.css"]')) {
  ...
  premiumTheme.href = "premium-pages.css?v=20260825-appleui3";
