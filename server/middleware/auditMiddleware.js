...
function details(req){
  ...
  if(path==="/api/admin-config/store")return{action:"store.configuration_updated",entity:"settings",target:"store",summary:"Store and payment configuration updated.",metadata:{storeOpen:b.storeOpen===true,purchasesEnabled:b.purchasesEnabled===true,manualBankEnabled:b.manualBankEnabled===true,onlinePaymentEnabled:b.onlinePaymentEnabled===true}};
  if(path==="/api/admin-config/bank-details")return{action:"payment.bank_details_updated",entity:"settings",target:"manual-payment",summary:"Manual-payment bank details updated.",metadata:{bankName:clean(b.bankName,100),accountNumberLast4:clean(b.bankAccountNumber).slice(-4)}};
  if(path==="/api/admin-config/staff"&&method==="POST")return{action:"staff.created",entity:"security",target:clean(b.email),summary:"Staff account created.",metadata:{permissions:Array.isArray(b.permissions)?b.permissions:[]}};
  if(path.includes("/api/admin-config/staff/")&&method==="PATCH")return{action:"staff.permissions_updated",entity:"security",target:id,summary:"Staff permissions or status updated.",metadata:{permissions:Array.isArray(b.permissions)?b.permissions:[],isActive:b.isActive===true}};
  if(path.includes("/api/admin-config/customers/")&&path.endsWith("/status"))return{action:b.isActive===true?"customer.reactivated":"customer.suspended",entity:"customer",target:id,summary:b.isActive===true?"Customer reactivated.":"Customer suspended and sessions ended.",metadata:{}};
  if(path.includes("/api/admin-config/customers/")&&path.endsWith("/notes"))return{action:"customer.note_added",entity:"customer",target:id,summary:"Private internal customer note added.",metadata:{}};
  if(path.includes("/api/admin-config/customers/")&&path.endsWith("/notify"))return{action:"customer.notified",entity:"customer",target:id,summary:"Direct customer notification sent.",metadata:{title:clean(b.title,120)}};
