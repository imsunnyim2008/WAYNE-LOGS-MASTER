...
let token=localStorage.getItem("wayneToken")||"",currentUser=null,products=[],activePlatform="all",currentProduct=null,checkoutSelection=null,checkoutRequestId="",walletBalanceKobo=0,cart=[];
...
async function loadWalletBalance(){
  if(!token)return;
  try{const r=await fetch(API_URL+"/api/wallet",{headers:{Authorization:"Bearer "+token}}),d=await r.json();if(!r.ok)return;walletBalanceKobo=Number(d.wallet?.balanceKobo||0);const el=document.getElementById("sideWalletBalance");if(el)el.textContent=walletMoney(walletBalanceKobo);if(checkoutWalletBalance)checkoutWalletBalance.textContent=walletMoney(walletBalanceKobo)}catch{}
}
...
function renderCheckoutRelated(product){
  if(!checkoutRelated)return;
  const related=products.filter(item=>String(item._id)!==String(product._id)&&Number(item.stock)>0&&(platformKey(item.platform)===platformKey(product.platform)||String(item.type||"")===String(product.type||""))).slice(0,3);
  checkoutRelated.innerHTML=related.length?related.map(item=>`<button class="checkout-related-card" type="button" data-checkout-related="${esc(item._id)}"><span class="checkout-related-icon">${iconMarkup(item.platform,item.icon||"★","")}</span><span><strong>${esc(item.name)}</strong><small>${money(item.price)} · ${Number(item.stock)} available</small></span><b>›</b></button>`).join(""):'<p class="checkout-no-related">No similar products available right now.</p>';
}
function openCheckoutFor(id){
  ...
  checkoutSelection=product;checkoutRequestId=(globalThis.crypto?.randomUUID?.()||`buy_${Date.now()}_${Math.random().toString(36).slice(2)}`).replace(/[^A-Za-z0-9_-]/g,"_");setCartProduct(id,false);closeCart();closeProduct();
  checkoutArt.innerHTML=`<div class="checkout-visual-logo">${iconMarkup(product.platform,product.icon||"★","")}</div>`;
  checkoutProduct.innerHTML=`<h2>${esc(product.name)}</h2><div class="checkout-platform">${iconMarkup(product.platform,product.icon||"★","")}<span>${esc(product.platform||"Digital")}</span></div>`;
  checkoutStock.textContent=`${Number(product.stock)} available`;checkoutDetails.textContent=product.description||`${product.name}. ${deliveryText(product)}.`;checkoutEmail.textContent=currentUser.email||"Your account";checkoutTotal.textContent=money(product.price);checkoutWalletBalance.textContent=walletMoney(walletBalanceKobo);renderCheckoutRelated(product);checkoutMessage.className="simple-message";checkoutMessage.textContent="";payButton.disabled=false;payButton.innerHTML="<span>⚡</span> Buy Now";checkoutModal.classList.add("show");checkoutModal.setAttribute("aria-hidden","false");document.body.style.overflow="hidden"
}
...
async function pay(){
  ...
  const reviewMessage=`Product: ${checkoutSelection.name}\nAmount: ${money(checkoutSelection.price)}\nDelivery: ${deliveryText(checkoutSelection)}\nWallet balance: ${walletMoney(walletBalanceKobo)}\n\nPlease recheck these details before continuing. Completed wallet purchases cannot be reversed.`;
  const approved=typeof window.wayneConfirm==="function"?await window.wayneConfirm(reviewMessage,{title:"Review Purchase",confirmText:"Continue",cancelText:"Recheck"}):window.confirm(reviewMessage);
  if(!approved)return;
  payButton.disabled=true;payButton.textContent="Processing purchase...";showCheckoutMessage("Securely checking your wallet balance...","info");
  try{
    ...
    const order=data.order;localStorage.setItem("wayneLastOrderId",order._id);response=await fetch(`${API_URL}/api/orders/${order._id}/wallet/pay`,{method:"POST",headers:{Authorization:"Bearer "+token}});data=await response.json();if(!response.ok)throw new Error(data.message||"Wallet payment could not be completed.");clearCart();if(typeof window.waynePopup==="function")await window.waynePopup("Payment successful. Your purchase is ready in My Orders.","success","Purchase Complete");location.href=`orders.html?order=${encodeURIComponent(order._id)}&walletPurchase=success`
  }catch(error){showCheckoutMessage(`${error.message} Your order is saved and can be retried from My Orders.`,"error");if(typeof window.waynePopup==="function")window.waynePopup(error.message||"Wallet payment failed.","error","Purchase Failed");payButton.disabled=false;payButton.innerHTML="<span>⚡</span> Buy Now"}
}
closeProductBtn?.addEventListener("click",closeProduct);closeCheckoutBtn?.addEventListener("click",closeCheckout);modalAdd?.addEventListener("click",()=>{if(currentProduct){setCartProduct(currentProduct._id,true);closeProduct()}});modalBuy?.addEventListener("click",()=>{if(currentProduct)openCheckoutFor(currentProduct._id)});payButton?.addEventListener("click",pay);checkoutRelated?.addEventListener("click",event=>{const card=event.target.closest("[data-checkout-related]");if(card)openCheckoutFor(card.dataset.checkoutRelated)});
