(function(){
  ...
  creditForm.addEventListener("submit",async event=>{event.preventDefault();const email=creditEmail.value.trim().toLowerCase(),amount=Number(creditAmount.value),reason=creditReason.value.trim();if(!confirm(`Credit ${email} with ₦${amount.toLocaleString("en-NG")} for “${reason}”? This will be recorded permanently.`))return;creditButton.disabled=true;creditButton.textContent="Crediting...";try{const requestId=globalThis.crypto?.randomUUID?.()||Date.now()+"-"+Math.random().toString(16).slice(2);await request("/api/wallet/admin/credit",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email,amount,reason,requestId})});alert("Wallet credited successfully.");creditEmail.value="";await loadWallets();await loadBalances()}catch(e){alert(e.message)}finally{creditButton.disabled=false;creditButton.textContent="Credit Wallet"}});
